...
def _fila(children):
    return html.Div(children, style={"display": "flex", "flexWrap": "wrap", "alignItems": "flex-end", "gap": "4px", "marginBottom": "6px"})
        ...
        [
            html.Div(
                [
                    ...
                    html.Details(
                        [
                            html.Summary("Cargar red"),
                            _fila([
                                html.Button("Red de ejemplo", id="ed-btn-ejemplo", n_clicks=0),
                            ]),
                            _fila([
                                _campo("Código SimBench", "ed-simbench-code", "1-LV-rural1--0-no_sw", tipo="text"),
                                html.Button("Cargar SimBench", id="ed-btn-simbench", n_clicks=0),
                            ]),
                            _fila([
                                dcc.Dropdown(id="ed-guardadas", placeholder="Red guardada…", style={"flex": "1", "minWidth": "180px"}),
                                html.Button("Cargar guardada", id="ed-btn-guardada", n_clicks=0),
                            ]),
                        ],
                        open=True,
                    ),
                    ...
                    html.Details(
                        [
                            ...
                            _fila([
                                ...
                                html.Button("+ Batería", id="ed-btn-bat", n_clicks=0),
                            ]),
                            _fila([
                                _campo("Red externa bus", "ed-ext-bus", 0),
                                html.Button("+ Red externa", id="ed-btn-ext", n_clicks=0),
                            ]),
                            _fila([
                                _campo("Quitar tipo", "ed-rm-type", "line", tipo="text"),
                                _campo("índice", "ed-rm-index", 0),
                                html.Button("Quitar", id="ed-btn-rm", n_clicks=0),
                            ]),
                        ]
                    ),
                    # ---- Modo código ----
                    html.Details(
                        [
                            html.Summary("Editor de código Python"),
                            html.P("Disponibles: model (NetworkModel), pp, Bus, Line, Load, SolarPanel, Battery, ExternalGrid, Transformer.",
                                   style={"fontSize": "11px", "color": "#666"}),
                            dcc.Textarea(
                                id="ed-code",
                                value="# Ejemplo:\n# model.add_bus(Bus(index=3, vn_kv=0.4, name='Nodo 3'))\n# model.add_line(Line(from_bus=2, to_bus=3, length_km=0.1))\n",
                                style={"width": "100%", "height": "120px", "fontFamily": "monospace"},
                            ),
                            html.Button("Ejecutar código", id="ed-btn-code", n_clicks=0),
                        ]
                    ),
                    # ---- Guardar ----
                    html.Details(
                        [
                            html.Summary("Guardar red"),
                            _fila([
                                _campo("Nombre", "ed-save-name", "Mi red", tipo="text"),
                                html.Button("Guardar como nueva", id="ed-btn-save", n_clicks=0),
                                html.Button("Guardar cambios", id="ed-btn-save-changes", n_clicks=0),
                            ]),
                        ]
                    ),
                    html.Div(id="ed-status", style={"marginTop": "8px", "fontWeight": "bold", "minHeight": "20px"}),
                    html.Pre(id="ed-summary", style={"fontSize": "12px", "background": "#f5f5f5", "padding": "8px", "borderRadius": "4px"}),
                ],
                style={"flex": "1", "minWidth": "340px", "maxWidth": "460px", "padding": "8px", "overflowY": "auto", "maxHeight": "82vh"},
            ),
            html.Div(
                cyto.Cytoscape(
                    id="ed-graph",
